...
async function main() {
  ...
  try {
    ...
    if (!editorEmail || !editorPassword) {
      throw new Error(
        "EDITOR_EMAIL and EDITOR_PASSWORD environment variables are required",
      );
    }
    ...
    const year2026 = await prisma.year.upsert({
      where: { name: "2026" },
      update: {},
      create: {
        name: "2026",
    ...
    console.log("Year created/updated:", year2026.name);
    ...
    if (!existingEditor) {
      ...
      const editorUser = await prisma.user.create({
        data: {
          ...
          role: "EDITOR",
          avatar:
            "https://images.pexels.com/photos/733872/pexels-photo-733872.jpeg?w=100&h=100&fit=crop",
        },
      });
    ...
    if (!existingSpeaker1) {
      speaker1 = await prisma.speaker.create({
        data: {
          ...
          photoUrl:
            "https://images.pexels.com/photos/1181686/pexels-photo-1181686.jpeg?w=400&h=400&fit=crop",
    ...
    if (!existingSpeaker2) {
      speaker2 = await prisma.speaker.create({
        data: {
          ...
          photoUrl:
            "https://images.pexels.com/photos/1181677/pexels-photo-1181677.jpeg?w=400&h=400&fit=crop",
          yearId: year2026.id,
        },
      });
    ...
    if (!existingAgenda1) {
      await prisma.agenda.create({
        data: {
          ...
          descriptionEn:
            "Explore how cloud computing is transforming businesses across Africa",
          descriptionFr:
            "Découvrez comment l'informatique en nuage transforme les entreprises à travers l'Afrique",
    ...
    if (!existingAgenda2) {
      await prisma.agenda.create({
        data: {
          ...
          descriptionEn:
            "Learn how to build scalable serverless applications using AWS Lambda and related services",
          descriptionFr:
            "Apprenez à créer des applications serverless évolutives en utilisant AWS Lambda et les services associés",
    ...
    if (!existingAgenda3) {
      await prisma.agenda.create({
        data: {
          ...
          descriptionEn:
            "Take a break, network with fellow attendees, and enjoy some refreshments",
          descriptionFr:
            "Prenez une pause, réseauter avec d'autres participants et profitez de rafraîchissements",
    ...
    if (existingGalleryImages.length === 0) {
      ...
      await prisma.galleryImage.createMany({
        data: [
          ...
          {
            imageUrl:
              "https://images.pexels.com/photos/1181686/pexels-photo-1181686.jpeg?w=600&h=400&fit=crop",
